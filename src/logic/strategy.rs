    ...
    for oger in &game_state.bases {
        ...
        for target in &game_state.bases {
            ...
            let mut sub_oger = PlayerAction {
                src: oger.uid,
                dest: target.uid,
                amount: target.population * 2 + std::cmp::max(0, distance(&target.position, &oger.position) - game_state.config.paths.grace_period) * game_state.config.paths.death_rate,
            };
